...
class ChannelConfigMigration:
    ...
    def _create_backup(self) -> Path:
        ...
        # Use .backup extension for consistency
    ...
    def _update_openclaw_config(self, merged_channels: Dict):
        ...
        for channel_type, channel_config in merged_channels.items():
            if channel_type in config['channels']:
                ...
                logger.warning(
                    f"Channel '{channel_type}' already exists in config, merging"
                )
        ...
        try:
            ...
            for config in channel_configs:
                ...
                if agent and agent.configuration and 'channels' in agent.configuration:
                    ...
                    # Update agent (SQLAlchemy will track the change)
...
def detect_conflicts(channel_configs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    # Build a map of all values for each field path
    field_values = {}
    ...
    def traverse(obj, path="", agent_info=None):
        ...
        # Lists are merged, not compared for conflicts
    ...
    # Find conflicts (fields with multiple different values)
    ...
    for key, value in update.items():
        if key in result:
            if isinstance(result[key], dict) and isinstance(value, dict):
                ...
                # Merge arrays by taking union (preserve order)
                seen = set(result[key])
                for item in value:
                    if item not in seen:
                ...
                # Scalar conflict - keep existing value (first encountered)
                pass
        else:
            result[key] = value
...
def apply_migration(
    ...
    Returns:
        MigrationStats with execution results
    """
