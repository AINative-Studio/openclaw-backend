    ...
    if (DBOS.isWithinWorkflow()) {
        if (DBOS.isInStep()) {
            ...
            if (stored !== undefined) {
                ...
                return dbos_executor_1.DBOSExecutor.reviveResultOrError(stored, executor.serializer);
            }
            return freshResult;
        }
        else {
...
class DBOS {
     ...
     */
    static async waitFirst(handles) {
        ...
        const completedId = await runInternalStep(async () => {
            return await dbos_executor_1.DBOSExecutor.globalInstance.systemDatabase.awaitFirstWorkflowId(workflowIds, DBOS.workflowID);
        }, 'DBOS.waitFirst');
    ...
    static async send(destinationID, message, topic, idempotencyKey, options) {
        ...
        const sermsg = (0, serialization_1.serializeValue)(message, DBOS.#executor.serializer, options?.serializationType ?? DBOS.defaultSerializationType);
        if (DBOS.isInWorkflow()) {
            const functionID = (0, context_1.functionIDGetIncrement)();
    ...
    static async recv(topic, timeoutSeconds) {
        ...
        if (DBOS.isWithinWorkflow()) {
            ...
            const msg = await dbos_executor_1.DBOSExecutor.globalInstance.systemDatabase.recv(DBOS.workflowID, functionID, timeoutFunctionID, topic, timeoutSeconds);
            return (0, serialization_1.deserializeValue)(msg.serializedValue, msg.serialization, DBOS.#executor.serializer);
        }
    ...
    static async setEvent(key, value, options) {
        ...
        if (DBOS.isWithinWorkflow()) {
            ...
            const functionID = (0, context_1.functionIDGetIncrement)();
            const serevt = (0, serialization_1.serializeValue)(value, DBOS.#executor.serializer, options?.serializationType ?? DBOS.defaultSerializationType);
    ...
    static async getEvent(workflowID, key, timeoutSeconds) {
        ...
        if (DBOS.isWithinWorkflow()) {
            ...
            const evt = await dbos_executor_1.DBOSExecutor.globalInstance.systemDatabase.getEvent(workflowID, key, timeoutSeconds ?? dbos_executor_1.DBOSExecutor.defaultNotificationTimeoutSec, params);
            return (0, serialization_1.deserializeValue)(evt.serializedValue, evt.serialization, DBOS.#executor.serializer);
        }
    ...
    static async writeStream(key, value, options = {}) {
        ...
        if (DBOS.isWithinWorkflow()) {
            const serval = (0, serialization_1.serializeValue)(value, DBOS.#executor.serializer, options.serializationType ?? DBOS.defaultSerializationType);
            if (DBOS.isInWorkflow()) {
                const functionID = (0, context_1.functionIDGetIncrement)();
    ...
    static async *readStream(workflowID, key) {
        ...
        while (true) {
            try {
                ...
                yield (0, serialization_1.deserializeValue)(value.serializedValue, value.serialization, dbos_executor_1.DBOSExecutor.globalInstance.serializer);
                offset += 1;
            }
            catch (error) {
    ...
    static async listSchedules(filters) {
        ...
        return results.map((r) => (0, scheduler_1.toWorkflowSchedule)(r, serializer));
    }
    static async getSchedule(name) {
        ...
        return result ? (0, scheduler_1.toWorkflowSchedule)(result, serializer) : null;
    }
